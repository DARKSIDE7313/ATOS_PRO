#!/usr/bin/env python3
# ⚠️ DEPRECATED — Phase 5 框架重塑 (2026-08-22) 归档。回测引擎 v1, 当前唯一活跃实现: atos/backtest/engine_v7.py。
...
def run_backtest(start_date='2018-01-01', end_date='2026-08-01', initial_capital=300000):
    ...
    for i, date in enumerate(trading_days):
        ...
        if len(equity_curve) > 0:
            ...
            if dd > PARAMS['max_drawdown_exit']:
                ...
                for sym in list(positions.keys()):
                    if sym in data and date in data[sym].index:
                        price = _next_open(data[sym], trading_days, i) or data[sym].loc[date, 'Close']
        ...
        for sym in list(positions.keys()):
            ...
            if sell_reason:
                fill = _next_open(data[sym], trading_days, i) or price
                fee = futu_sell_fee(pos['shares'], fill) if PARAMS['use_fees'] else 0
                total_fees += fee
                cash += pos['shares'] * fill - fee
                trades.append({
                    'date': str(date.date()), 'sym': sym, 'action': 'SELL',
                    'price': round(fill, 2), 'reason': sell_reason,
        ...
        for sym, score, price in candidates[:n_buys]:
            # M12: T 日信号 -> T+1 日开盘成交
            fill = _next_open(data[sym], trading_days, i)
            if fill is None:
                continue
            pos_value = port_value * PARAMS['single_position_pct']
            shares = max(1, int(pos_value / fill))
            fee = futu_buy_fee(shares, fill) if PARAMS['use_fees'] else 0
            cost = shares * fill + fee

            if cost > cash * (1 - PARAMS['min_cash_pct']):
                shares = max(1, int((cash * (1 - PARAMS['min_cash_pct'])) / fill))
                fee = futu_buy_fee(shares, fill) if PARAMS['use_fees'] else 0
                cost = shares * fill + fee
                if shares <= 0:
                    continue
            ...
            positions[sym] = {
                ...
                'buy_date': date, 'high_since_buy': fill,
            }
            trades.append({
                'date': str(date.date()), 'sym': sym, 'action': 'BUY',
                'price': round(fill, 2), 'score': round(score, 3),
                'shares': shares,
            })
    ...
    if len(equity_curve) > 1:
        ...
        sharpe = returns.mean() / returns.std() * np.sqrt(252) if returns.std() > 0 else 0  # M9: 日采样用 sqrt(252)（原 sqrt(252/5) 低估 ~2.24 倍）
    else:
        sharpe = 0
