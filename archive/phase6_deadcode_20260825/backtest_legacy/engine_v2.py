#!/usr/bin/env python3
# ⚠️ DEPRECATED — Phase 5 框架重塑 (2026-08-22) 归档。回测引擎 v2, 当前唯一活跃实现: atos/backtest/engine_v7.py。
...
def _next_open(df, dates, i):
    """M12: T+1 开盘价（消除未来函数）。无下一天返回 None。"""
...
def run_backtest_v2(start_date='2018-01-01', end_date='2026-08-01', initial_capital=300000):
    ...
    for sym in all_syms:
        try:
            hist = yf.Ticker(sym).history(start=start_date, end=end_date, auto_adjust=True)
            if len(hist) > 100:
                data[sym] = hist
        except Exception:
            pass
    ...
    for i, date in enumerate(trading_days):
        ...
        if i - last_rebalance >= 10 and i > 63:
            ...
            if bull:
                ...
                if core_etf not in positions and target_core_value > 1000:
                    # 买入 QQQ (M12: T+1 开盘成交)
                    price = _next_open(qqq, trading_days, i) or qqq_price
                ...
                for etf in list(positions.keys()):
                    if positions[etf].get('type') == 'core':
                        price = _next_open(data[etf], trading_days, i) or (data[etf].loc[date, 'Close'] if date in data[etf].index else positions[etf]['avg_price'])
        ...
        for sym in list(positions.keys()):
            ...
            if sell_reason:
                fill = _next_open(mom_data[sym], trading_days, i) or price
                fee = futu_sell_fee(pos['shares'], fill)
                total_fees += fee
                cash += pos['shares'] * fill - fee
                trades.append({
                    'date': str(date.date()), 'sym': sym, 'action': 'SELL',
                    'price': round(fill, 2), 'shares': pos['shares'],
        ...
        for sym, score, price in candidates[:n_buys]:
            # M12: T 日信号 -> T+1 日开盘成交
            fill = _next_open(mom_data[sym], trading_days, i)
            if fill is None:
                continue
            pos_value = alpha_budget / max_alpha
            shares = max(1, int(pos_value / fill))
            fee = futu_buy_fee(shares, fill)
            cost = shares * fill + fee

            if cost > cash * 0.90:
                shares = max(1, int(cash * 0.85 / fill))
                fee = futu_buy_fee(shares, fill)
                cost = shares * fill + fee
                if shares <= 0:
                    continue
            ...
            total_fees += fee
            positions[sym] = {
                'shares': shares, 'avg_price': fill,
                'buy_date': date, 'peak_price': fill, 'type': 'alpha',
            }
            trades.append({
                'date': str(date.date()), 'sym': sym, 'action': 'BUY',
                'price': round(fill, 2), 'shares': shares,
    ...
    if len(equity_curve) > 1:
        ...
        sharpe = rets.mean() / rets.std() * np.sqrt(252) if rets.std() > 0 else 0  # M9: 日采样用 sqrt(252)
    else:
        sharpe = 0
