        ...
        for i, date in enumerate(spy.index):
            ...
            if i - last_rebalance >= 63:
                ...
                for sym in list(positions.keys()):
                    price = _next_open(data[sym], spy.index, i)
                    if price is None:
                        del positions[sym]
                        continue
                ...
                # 买 QQQ 核心
                if cfg['core'] > 0:
                    q_price = _next_open(qqq, spy.index, i) or (qqq.loc[date, 'Close'] if date in qqq.index else core_val)
                ...
                if cfg['alpha_n'] > 0:
                    ...
                    for sym, score, price in candidates[:cfg['alpha_n']]:
                        fill = _next_open(data[sym], spy.index, i)
                        if fill is None:
                            continue
                        shares = max(1, int(per_stock / fill))
