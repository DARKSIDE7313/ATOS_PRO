...
class ShadowAccount:
    ...
    def is_cooling_off(self, symbol: str) -> bool:
        ...
        if symbol in self.stop_loss_blacklist:
            ...
            if self.cycle_count - sold_cycle < COOLDOWN_CYCLES:
                return True
            else:
    ...
    def add_to_blacklist(self, symbol: str):
        ...
        logger.info(f"🔒 冷却: {symbol} → 禁止买入至周期#{self.cycle_count + COOLDOWN_CYCLES}")

    def clean_blacklist(self):
        """清理过期条目"""
        expired = [s for s, c in self.stop_loss_blacklist.items()
                   if self.cycle_count - c >= COOLDOWN_CYCLES]
        for s in expired:
    ...
    def execute(self, symbol: str, action: str, shares: int,
        ...
        # 单仓上限
        max_pos_val = self.total_equity * self.max_single_pct
        current_val = self.positions[symbol]["qty"] * price if symbol in self.positions else 0
        max_buy = max_pos_val - current_val
...
def run_shadow_cycle(account: ShadowAccount, cycle: int = 0):
    ...
    # ---- 3. 因子 ----
    symbols = sorted(signals.keys(), key=lambda s: signals[s].get("score", 0), reverse=True)[:20]
    top_picks = []
    factor_result = {}
    try:
    ...
    # 4b. 追踪止损（每个标的独立判断）
    # v4: BEAR/CAUTIOUS 趋势下关闭追踪止损 — 下跌趋势不止损，等反弹
    ...
    for sym, pos in list(account.positions.items()):
        ...
        if sym not in account.trailing_stops:
            if not use_trailing:
                continue  # BEAR 不创建新的追踪止损
            # 波动率追踪止损（v4: 确认周期5次=25分钟，过滤假突破）
            atr_val = signals.get(sym, {}).get("atr", 0)
            trail = max(0.04, min(0.12, (atr_val / price) * 4)) if atr_val > 0 else 0.05
            ts = TrailingStop(trail_pct=trail, confirm_cycles=5)  # 5次确认=25分钟
            ts.init(pos["avg_price"])
            account.trailing_stops[sym] = ts
            continue

        if not use_trailing:
            # BEAR 趋势：不清除已有追踪止损，但不运行更新检查
            # （已有持仓就不动，等回到 BULL 再激活）
            continue
...
def _factor_based_buying(account, signals, top_picks, factor_result, regime, spy_trend, gate_exposure=1.0, ai_veto_map=None):
    ...
    for pick in candidates:
        ...
        # RSI过滤（放宽: 65→75, 给强势股更多空间。真正的超买区在85以上才剔除）
        rsi = signals.get(sym, {}).get("rsi", 50)
        if rsi > 75:
            logger.debug(f"⏭ {sym} RSI={rsi:.0f}>75 超买")
            continue

        # MA200偏离过滤（放宽: 10%→25%, 牛市行情中优质股经常在MA200上方+10~20%运行）
...
def _finalize_cycle(account, cycle, regime, current_vix, signals, top_picks,
    ...
    """每个周期结束前的最终处理"""
    # 记录周期收益 — 使用 equity_history 来精确计算变化
    prev_eq = None
    if account.equity_history:
        last_eh = account.equity_history[-1]
        if isinstance(last_eh, dict):
            prev_eq = last_eh.get("equity")
    prev_equity = prev_eq if prev_eq is not None else account.initial_cash
    cycle_ret = (account.total_equity - prev_equity) / prev_equity if prev_equity > 0 else 0
    account.cycle_returns.append(round(cycle_ret, 6))
    # 清洗 cycle_returns 中的 nan（从旧 state 继承的垃圾）
    account.cycle_returns = [
        0.0 if (isinstance(r, float) and str(r) == "nan") else r
        for r in account.cycle_returns
    ]
    ...
    logger.info(f"Cycle {cycle} done | Equity=${account.total_equity:,.0f} | "
