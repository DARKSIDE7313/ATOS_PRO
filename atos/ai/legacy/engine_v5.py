"""
⚠️ DEPRECATED — Phase 5 框架重塑 (2026-08-22) 归档。AI 引擎 v5 (报告 §7.2: 5代重复之一)。
当前活跃实现: atos/ai/advisor_enhanced.py (v6, shadow_trader 唯一调用)。
    ...
    try:
        ...
        for d in debate_results:
            if d.get("final_verdict") == "BUY" and d.get("confidence", 0) < MIN_CONFIDENCE_FOR_BUY:
                ...
                d["reasoning"] += f" [置信度{d.get('confidence',0):.2f}<{MIN_CONFIDENCE_FOR_BUY}，降级WAIT]"
