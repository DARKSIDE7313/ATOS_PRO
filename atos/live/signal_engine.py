...
# L1: 移除全局 socket.setdefaulttimeout(8) — 进程级副作用影响其它库，yfinance 已显式传 timeout=8
...
def _prefetch_batch(symbols: list[str]) -> None:
    """⚠️ 死代码（L2 标注，有意禁用）: 中国大陆 yfinance 被墙，批量下载已停用。

    本函数直接 return，_batch_success 恒为 False，因此
    _get_cached_data() 中 `if _batch_success:` 分支为死代码。
    单只标的走 _get_cached_data 内的 Futu 历史数据路径。保留仅为接口兼容，勿在此添加逻辑。
    """
...
def get_realtime_signals(symbols: list[str] = None,
    ...
    if not signals and _signal_cache:
        # L3: 周末不回退到周五缓存（过期价会污染卖出检查）
        from datetime import timezone
        if datetime.now(timezone.utc).weekday() >= 5:
            logger.info("⏸ 周末休市，不使用过期信号缓存")
            return {}
