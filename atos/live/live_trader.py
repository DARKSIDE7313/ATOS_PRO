...
def is_market_open():
    """美股交易时间 — M4: 复用 futu_bridge 的统一时钟（正确处理 DST + 假日）"""
    from atos.live.futu_bridge import is_market_open as _futu_open
    return _futu_open()[0]
...
def get_regime():
    ...
    engine.spy_prices = []
    engine.vix_prices = []
    # M14: MultiIndex 兼容 — yf.download 可能返回 MultiIndex columns
