#!/usr/bin/env python3
# ⚠️ DEPRECATED — Phase 5 框架重塑 (2026-08-22) 归档。回测引擎 v5, 当前唯一活跃实现: atos/backtest/engine_v7.py。
...
def run_strategy(data, dates, name, core_pct=0.60, n_stocks=5, use_rs=False, vol_weight=False, dual_mom=False, sector_mom=False):
    ...
    for i, date in enumerate(dates):
        ...
        if i % 63 != 0:  # 季度再平衡（M10: 移除 i<200 双重预热，dates 已切片）
            continue
        ...
        for sym in list(positions.keys()):
            if sym not in targets:
                ...
                price = _next_open(data[sym], dates, i)
                if price is None:
                    continue
    ...
    years = len(data['SPY']) / 252  # M11: 用完整交易日数（原 len(dates) 已切片，少算 ~0.8 年致年化高估）
