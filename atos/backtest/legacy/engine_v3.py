#!/usr/bin/env python3
# ⚠️ DEPRECATED — Phase 5 框架重塑 (2026-08-22) 归档。回测引擎 v3, 当前唯一活跃实现: atos/backtest/engine_v7.py。
...
def run_v3(start_date='2016-01-01', end_date='2026-08-01', capital=300000):
    ...
    tlt = yf.Ticker('TLT').history(start=start_date, end=end_date, auto_adjust=True)
    ...
    for strat_name, cfg in strategies.items():
        ...
        for i, date in enumerate(spy.index):
            ...
            q_fill = _next_open(qqq, spy.index, i) or q_price  # M12: T+1 开盘成交
            t_fill = _next_open(tlt, spy.index, i) or (tlt.loc[date, 'Close'] if date in tlt.index else q_price)
            ...
            if in_position and shares > 0:
                ...
                if cfg.get('trailing_stop', 0) > 0:
                    ...
                    if ts_drop >= cfg['trailing_stop']:
                        # 止损卖出
                        fee = futu_sell_fee(shares, q_fill)
                        total_fees += fee
                        cash += shares * q_fill - fee
                        pnl = (q_fill - avg_price) / avg_price
                        trades.append({'date': str(date.date()), 'action': 'SELL', 'price': round(q_fill, 2), 'reason': f'TS{ts_drop:.1%}', 'pnl': round(pnl * 100, 2)})
                        shares = 0
                        in_position = False
                        peak_price = 0
            ...
            if not bull and in_position and shares > 0:
                ...
                cash += shares * q_fill - fee
                pnl = (q_fill - avg_price) / avg_price
                trades.append({'date': str(date.date()), 'action': 'SELL', 'price': round(q_fill, 2), 'reason': 'BEAR', 'pnl': round(pnl * 100, 2)})
                shares = 0
                in_position = False
                peak_price = 0
            ...
            if bull and not in_position and shares == 0 and bear_shares == 0:
                # 买入 QQQ
                shares = max(1, int(cash * 0.95 / q_fill))
                fee = futu_buy_fee(shares, q_fill)
                total_fees += fee
                cost = shares * q_fill + fee
                if cost <= cash:
                    cash -= cost
                    avg_price = q_fill
                    peak_price = q_fill
                    in_position = True
                    trades.append({'date': str(date.date()), 'action': 'BUY', 'price': round(q_fill, 2), 'shares': shares})
                else:
                    shares = 0
